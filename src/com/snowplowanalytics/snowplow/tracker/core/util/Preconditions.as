package com.snowplowanalytics.snowplow.tracker.core.util
{
	...
	*/
	...
	public class Preconditions {
		 ...
		 */
		public static function checkArgument(expression:Boolean, ... errorMessageArgs:*):void {
		  if (!expression) {
			if (errorMessageArgs != null && errorMessageArgs.length == 1) {
  	          throw new Error(String(errorMessageArgs[0]));
			} else if (errorMessageArgs != null && errorMessageArgs.length > 1) {
			  var errorMessageTemplate:String = errorMessageArgs[0];
			  var args:Array = errorMessageArgs.slice(1);
			  throw new Error(format(errorMessageTemplate, errorMessageArgs));
			} else {
		 ...
		 *     {@code errorMessageArgs} is null (don't let this happen)
		 */
		public static function checkState(expression:Boolean, ... errorMessageArgs:*):void {
 		  if (!expression) {
			  if (errorMessageArgs != null && errorMessageArgs.length == 1) {
				  throw new Error(String(errorMessageArgs[0]));
			  } else if (errorMessageArgs != null && errorMessageArgs.length > 1) {
		 ...
		 */
		public static function checkNotNull(reference:*, ... errorMessageArgs:*):* {
			if (reference == null) {
				if (errorMessageArgs != null && errorMessageArgs.length == 1) {
					throw new Error(String(errorMessageArgs[0]));
				} else if (errorMessageArgs != null && errorMessageArgs.length > 1) {
		...
		*/
		 ...
		 */
		public static function checkElementIndex(index:int, size:int, desc:String = "index"):int {
				// Carefully optimized for execution by hotspot (explanatory comment above)
				if (index < 0 || index >= size) {
					throw new Error(badElementIndex(index, size, desc));
				}
				return index;
			}
		
		private static function badElementIndex(index:int, size:int, desc:String):String {
			if (index < 0) {
		 ...
		 * @throws IllegalArgumentException if {@code size} is negative
		 */
		public static function checkPositionIndex(index:int, size:int, desc:String = "index"):int {
			// Carefully optimized for execution by hotspot (explanatory comment above)
			if (index < 0 || index > size) {
				throw new Error(badPositionIndex(index, size, desc));
			}
			return index;
		}
		
		private static function badPositionIndex(index:int, size:int, desc:String):String {
			if (index < 0) {
				return format("%s (%s) must not be negative", desc, index);
			} else if (size < 0) {
				throw new Error("negative size: " + size);
		...
		public static function checkPositionIndexes(start:int, end:int, size:int):void {
			...
			if (start < 0 || end < start || end > size) {
				throw new Error(badPositionIndexes(start, end, size));
			}
		}
		
		private static function badPositionIndexes(start:int, end:int, size:int):String {
		...
		private static function format(template:String, ... args:*):String {
			template = String(template); // null -> "null"
			...
			var i:int = 0;
			while (i < args.length) {
				var placeholderStart:int = template.indexOf("%s", templateStart);
				if (placeholderStart == -1) {
					break;
				}
				builder += template.substring(templateStart, placeholderStart);
				builder += String(args[i++]);
				
				templateStart = placeholderStart + 2;
			}
			builder += template.substring(templateStart);
